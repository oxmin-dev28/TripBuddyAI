    ...
    if (result.rows.length === 0) {
      ...
      result = await query(
        `INSERT INTO users (id, email, name)
         VALUES ($1, $2, $3)
         RETURNING *`,
        [uuidv4(), email, name || email.split('@')[0]]
      );
    }
...
router.post('/:id/visits', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `INSERT INTO visits (id, user_id, place_id, rating)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [uuidv4(), id, placeId, rating || null]
    );
...
router.get('/:id/visits', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT * FROM visits WHERE user_id = $1 ORDER BY visited_at DESC`,
      [id]
    );
