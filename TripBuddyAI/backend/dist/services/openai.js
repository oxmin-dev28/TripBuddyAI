...
function enrichPlanWithPlaces(days, nearbyPlaces) {
    ...
    return days.map(day => ({
        ...
        slots: day.slots.map(slot => ({
            ...
            options: slot.options.map(option => {
                ...
                const matchingPlace = nearbyPlaces.find(p => p.name.toLowerCase().includes(option.name.toLowerCase()) ||
...
function generateDefaultPlan(preferences, places) {
    const days = [];
